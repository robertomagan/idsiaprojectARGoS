...
{
  _HRVOAgent->velocity_=HRVO::Vector2((float)velocity.GetX(),(float)velocity.GetY());
  _HRVOAgent->orientation_=angle.SignedNormalize().GetValue();
  _HRVOAgent->position_=HRVO::Vector2((float)position.GetX(),(float)position.GetY());
...
{  
    HRVO::Vector2 t=HRVO::Vector2((float)targetPosition.GetX(),(float)targetPosition.GetY())-_HRVOAgent->position_;
...
void HRVOAgent::updateVelocity()
 {
   Agent::updateVelocity();
 }

void HRVOAgent::Init(TConfigurationNode& t_tree)
 {
   Agent::Init(t_tree);
   ...
   printf("HRVO Agent INIT: radius %.5f\n", _HRVOAgent->radius_);
...
void HRVOAgent::addObstacleAtPoint(CVector2 p,Real r)
{
