...
static CVector2 relax(CVector2 x0,CVector2 x1, Real tau);
static Real relax(Real x0,Real x1, Real tau);
...
void HLAgent::addObstacleAtPoint(CVector2 point,Real radius)
  {
    AgentCache obstacle=makeObstacleAtPoint(point,radius);
...
std::vector<Real> HLAgent::getDistances()
{
 ...
 void HLAgent::updateDesideredVelocity()
  {
    setup();
    ...
    //printf("HLAgent: updateDesideredVelocity to %.2f, h = %.2f \r\n",a0.GetValue(),horizon);
    ...
    desideredAngle=nearestAngle;
    desideredSpeed=newTargetSpeed;
    desideredVelocity=CVector2(newTargetSpeed,nearestAngle);

    //DEBUG_CONTROLLER("=> desideredVelocity (%.2f,%.2f) %.2f \n",desideredVelocity.GetX(),desideredVelocity.GetY(),desideredAngle.GetValue());

    //    DEBUG_CONTROLLER("=> desidered speed %.2f, desidered angle %.2f \n",desideredSpeed,desideredAngle.GetValue());
...
void HLAgent::addObstacleAtPoint(CVector2 p,CVector2 v,Real r)
{
  ...
  AgentCache agent=makeObstacleAtPoint(p,v,r);
...
AgentCache HLAgent::makeObstacleAtPoint(CVector2 p,Real r)
  {
...
AgentCache HLAgent::makeObstacleAtPoint(CVector2 obstaclePosition,CVector2 obstacleVelocity, Real obstacleRadius)
  {
    AgentCache agent=AgentCache();
    agent.radius=radius;
    Real distance;
  ...
  void HLAgent::updateVelocity()
  {
    ...
    // printf("desidered lin speed %.3f, des angle %.3f, optimal speed %.3f\n",desideredSpeed,desideredAngle.GetValue(),optimalSpeed);
    
    Agent::updateVelocity();

    //    printf("=> L %.3f -> %.3f R %.3f -> %.3f (%.3f)\n",leftWheelSpeed,leftWheelTargetSpeed,rightWheelSpeed,rightWheelTargetSpeed,tau);

    leftWheelTargetSpeed=relax(leftWheelSpeed,leftWheelTargetSpeed,tau);
    rightWheelTargetSpeed=relax(rightWheelSpeed,rightWheelTargetSpeed,tau);  
   
    leftWheelSpeed=leftWheelTargetSpeed;
    rightWheelSpeed=rightWheelTargetSpeed;
...
void HLAgent::Init (TConfigurationNode& t_tree)
{
  Agent::Init(t_tree);
  ...
  if(NodeExists(t_tree,"aperture"))
    {
      GetNodeText(GetNode(t_tree, "aperture"), text);
      Real a;
      sscanf(text.c_str(),"%f",&a);
      aperture=CRadians(a);
    }
  if(NodeExists(t_tree,"resolution"))
    {
      GetNodeText(GetNode(t_tree, "resolution"), text);
      sscanf(text.c_str(),"%d",&resolution);
      resolution=fmin(MAX_RESOLUTION,resolution);
    }
  if(NodeExists(t_tree,"tau"))
    {
      GetNodeText(GetNode(t_tree, "tau"), text);
      sscanf(text.c_str(),"%f",&d);
      tau=fmax(MIN_TAU,d);
    }
  if(NodeExists(t_tree,"eta"))
    {
      GetNodeText(GetNode(t_tree, "eta"), text);
      sscanf(text.c_str(),"%f",&d);
      eta=fmax(tau,d);
    }

  //  printf("*\n");
  printf("HLAgent Init: Tau %.2f Eta %.2f Resolution %d Aperture %.2f \n",tau,eta,resolution,aperture.GetValue());
  //  printf("!\n");
}
  ...
  void HLAgent::updateVelocityCartesian ()
  {
    ...
    //Perhaps it makes more sense to do it in polor coordinates, i.e speed->exp(...), angle->angle(...)

    desideredVelocity=relax(velocity,desideredVelocity,tau);
    //desideredAngle=relax(0,desideredAngle,tau);
#endif
    
    CRadians delta;

    Real v=fabs(desideredVelocity.Length()/0.05);

    if(v>1) delta=desideredVelocity.Angle();
    else delta=(CVector2(v,desideredVelocity.Angle())+CVector2(1-v,desideredAngle)).Angle();
	...
	angularSpeed=-maxRotationSpeed;
      ...
      {
	linearSpeed=desideredVelocity.Length()*(1-fabs(angularSpeed)/maxRotationSpeed);
      }
    //DEBUG_CONTROLLER("Angular Speed %.2f, Linear Speed %.2f [cm/s]\r\n",100*angularSpeed,100*linearSpeed);
    leftWheelTargetSpeed=linearSpeed-angularSpeed;
    rightWheelTargetSpeed=linearSpeed+angularSpeed;
  }
